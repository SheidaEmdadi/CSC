...
we have prepared a [curriculum package](/src/Curriculum.zip).
...
5. [Teaching Guide](/src/Curriculum.md).
...
- [Design a](/src/3-PartyGame/wrong-party1.json) (❌ Missing synchronizing messages.)
- [Design b](/src/3-PartyGame/wrong-party2.json) (❌ Wrong transition in the global statechart; there should be a way to get from the `waiting` state to the `PartyStarted` instead of from `PartyCancelled`.)
- [**Design c**](/src/3-PartyGame/PartyGame.json) (✔ Correct)
- [Design d](/src/3-PartyGame/wrong-party3.json) (❌ The local messages should be connected to the user's channel (they are generated by user interaction.))
